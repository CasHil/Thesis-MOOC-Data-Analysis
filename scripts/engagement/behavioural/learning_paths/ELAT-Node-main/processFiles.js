...
async function readMetadataFiles(directoryPath) {
  ...
  fs.readdir(directoryPath, (err, files) => {
    ...
    files.forEach(file => {
      ...
      if (file.includes(sqlType) || file.includes(jsonType) || file.includes(mongoType)) {
        fs.readFile(filePath, 'utf8', (err, content) => {
          ...
          fileNames = fileNames + file + ' size: ' + content.length + ' bytes \n';
          counter++;
...
async function processMetadataFiles(files) {
  ...
  if (Object.keys(courseMetadataMap).length < 1) {
      ...
      await mongoInsert('webdata', [{'name': 'segmentation', 'object': segmentationType}]);
      ...
      if (! requiredFiles.every(function(x) { return x in fileMap; })) {
        ...
        await mongoInsert('metadata', [{'name': 'metadata_map', 'object': courseMetadataMap}]);
      }
    }
  };
...
module.exports = readMetadataFiles("W:/staff-umbrella/gdicsmoocs/Working copy/EX101x_2T2018_run6 - Copy");
